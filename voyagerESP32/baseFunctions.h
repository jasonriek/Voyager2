...
void handleDeviceInfo(){
  ...
  Serial.println(responseJsonWeb);
...
void webMotorCtrl(float inputL, float inputR){
  ...
  lastCmdTime = millis();
  if(spdSetWeb == 0){
    rightCtrl(64 * inputR * rightRate + 0.5);
     leftCtrl(64 * inputL * leftRate + 0.5);
  }
  else if(spdSetWeb == 1){
    rightCtrl(156 * inputR * rightRate + 0.5);
     leftCtrl(156 * inputL * leftRate + 0.5);
  }
  else if(spdSetWeb == 2){
